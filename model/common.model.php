...
//    $sql = "SELECT * FROM ( SELECT a.goods_name AS goods_name, MIN(sku.to_c_adult_price) AS min_price, a.goods_image AS goods_image, b.`addtime` AS browse_time, b.`browse_id` AS browse_id, b.goods_id AS goods_id, a.goods_type AS goods_type, a.`lv_product_id` AS lv_scenic_id FROM `t_goods_thread` a, ( SELECT * FROM `t_goods_browse` t WHERE t.`site_id` = '$g_siteid' -- AND (t.`session_id` = 'va774fkr72pftmclfn135c0dm5'OR t.`user_id` = '-1')
...
FROM
	(
		SELECT
			...
			b.`addtime` AS browse_time,
			b.`browse_id` AS browse_id,
		...
		UNION
			SELECT
				...
				tb.`addtime` AS browse_time,
				tb.`browse_id` AS browse_id,
				tg.lv_product_id AS goods_id,
				tg.goods_type AS goods_type,
				tg.lv_scenic_id AS lv_scenic_id
			FROM
				t_goods_thread tg
			LEFT JOIN t_goods_browse tb ON tb.goods_id = tg.goods_id
			WHERE
			tg.serach_flag = 1 
			AND tg.goods_type = 4
			AND tb.`site_id` = '$g_siteid' 
	) AS aaa
ORDER BY
	browse_id DESC
LIMIT 0, $limit;
