...
if(req('kw')!=''){
	$qer .= " AND (b.`goods_name` LIKE '%".req('kw')."%' OR a.`order_code`='".req('kw')."')";
}

$sql = "SELECT a.*, b.goods_id, b.goods_name FROM `t_user_order_tourist` a, t_user_order b WHERE a.order_id=b.order_id AND a.`site_id`='$g_siteid' $qer ORDER BY a.order_code DESC, a.`tourist_id` DESC";
