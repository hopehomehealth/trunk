...
if ($output['status'] != "0000") {
    $sql = "UPDATE `t_refund_fee_apply` SET `flag` = '4',`deduct_fee` ='$cutpay' WHERE `order_code` = '$orderno'";
    $db->query($sql);
    echo "<script>alert('退款失败！')</script>";
    $url = "./?cmd=".base64_encode("order_refund.php");
    gourl($url);
} else {
    $sql = "UPDATE `t_refund_fee_apply` SET `flag` = '2',`deduct_fee` ='$cutpay'  WHERE `order_code` = '$orderno'";
    $db->query($sql);
