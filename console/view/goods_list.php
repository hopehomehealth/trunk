		...
		<tr>
			<td style="padding-top:7px">
				<form name="q_from" method="GET" action="" class="form-inline">  
					...
					<option value="0" <? if(req('sale_type')=='0') echo 'selected';?>>普通</option> 
					<option value="1" <? if(req('sale_type')=='1') echo 'selected';?>>团购</option> 
					<option value="2" <? if(req('sale_type')=='2') echo 'selected';?>>抢购</option> 
					</select>
			...
			<tr <?if($val['is_sale']=='0'){?>class="error" title="已下架"<?}?>> 
			  ...
			  <td >
				<select name="is_hot" onchange="doform_goods('<?=$val['goods_id']?>', 'is_hot')" style="width:80px;" >  
					...
					<option value="2" <? if($val['is_hot']==2) {echo 'selected';} ?> >推荐到全国^</option>
				</select>
			  </td> 
